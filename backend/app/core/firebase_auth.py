...
def require_admin(user: dict = Depends(get_firebase_user)):
    ...
    """
    # Check for direct 'admin' claim or role in user object
    is_admin = user.get("admin") is True or user.get("role") == "admin"

    # Fallback/Greylist for specific dev emails
    if not is_admin:
        email = user.get("email", "").lower()
        if (
            email.endswith("@tnimpact.com")
            or email in {"sureshkumar@gmail.com", "varshini@gmail.com", "admin@tnimpact.com", "admin@gmail.com"}
            or "admin" in email
        ):
            is_admin = True
...
def require_driver(user: dict = Depends(get_firebase_user)):
    """
    Dependency to restrict access to Driver users.
    A user can be BOTH admin and driver in edge cases.
    """
    is_driver = user.get("driver") is True or user.get("role") == "driver"
    
    # If not explicitly marked as driver, check if they are an admin (Admins can act as drivers)
    if not is_driver:
        try:
            require_admin(user)
            is_driver = True
